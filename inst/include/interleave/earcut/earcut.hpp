...
private:
  struct Node {
    ...
    const N i;
    const std::vector< double > coords;  // must have at least 2 dimensions
  ...
  template <typename Polygon> void earcutLinked(Node* ear, const Polygon& points, int pass = 0);
  bool isEar(Node* ear);
  bool isEarHashed(Node* ear);
  Node* cureLocalIntersections(Node* start);
  template <typename Polygon> void splitEarcut(Node* start, const Polygon& points);
...
template <typename N> template <typename Polygon>
void Earcut<N>::earcutLinked(Node* ear, const Polygon& points, int pass) {
  if (!ear) return;
  ...
  while (ear->prev != ear->next) {
    ...
    if (hashing ? isEarHashed(ear) : isEar(ear)) {
      ...
      for( std::size_t i = 0; i < p_coords.size(); ++i ) {
        xyzcoords.emplace_back( p_coords[i] );
      }

      for( std::size_t i = 0; i < e_coords.size(); ++i ) {
        xyzcoords.emplace_back( e_coords[i] );
      }

      for( std::size_t i = 0; i < n_coords.size(); ++i ) {
        xyzcoords.emplace_back( n_coords[i] );
      }
    ...
    if (ear == stop) {
      ...
      if (!pass) earcutLinked(filterPoints(ear), points, 1);

      // if this didn't work, try curing all small self-intersections locally
      else if (pass == 1) {
        ear = cureLocalIntersections(filterPoints(ear));
        earcutLinked(ear, points, 2);

        // as a last resort, try splitting the remaining polygon into two
      } else if (pass == 2) splitEarcut(ear, points);
...
Earcut<N>::cureLocalIntersections(Node* start) {
  ...
  do {
    ...
    if (!equals(a, b) && intersects(a, p, p->next, b) && locallyInside(a, b) && locallyInside(b, a)) {
      ...
      xyzcoords.emplace_back(p->x);
      xyzcoords.emplace_back(p->y);

      xyzcoords.emplace_back(b->x);
      xyzcoords.emplace_back(b->y);
...
template <typename N> template <typename Polygon>
void Earcut<N>::splitEarcut(Node* start, const Polygon& points) {
  ...
  do {
    ...
    while (b != a->prev) {
      if (a->i != b->i && isValidDiagonal(a, b)) {
        ...
        earcutLinked(a, points);
        earcutLinked(c, points);
        return;
      }
      b = b->next;
    }
...
Earcut<N>::splitPolygon(Node* a, Node* b) {
  ...
  Node* a2 = nodes.construct(a->i, a->coords);
  Node* b2 = nodes.construct(b->i, b->coords);
...
Earcut<N>::insertNode(std::size_t i, const Point& pt, Node* last) {
  ...
  Node* p = nodes.construct( static_cast<N>(i), pt );
