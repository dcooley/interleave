...
inline SEXP interleave_triangle(
  ...
  //Rcpp::IntegerVector input_indices; // filled later if required
  ...
  for( i = 0; i < n; ++i ) {
    ...
    // Rcpp::Rcout << "nv.length() " << nv.length() << std::endl;
    // Rcpp::Rcout << "iv.length() " << iv.length() << std::endl;
  ...
  SEXP ri = interleave::utils::unlist_list( res_indices ); // can't unlist_list this if it's NULL
  Rcpp::IntegerVector input_indices = Rcpp::as< Rcpp::IntegerVector >( ri );
  ...
  if( shuffle_properties ) {
    ...
    for( i = 0; i < n_properties; ++i ) {
      ...
      res_properties[ i ] = subset_vector( p, input_indices );
    }
  }
  ...
  //   Rcpp::stop("interleave - geometry coordinates length 0");
  // }
  ...
  // Rcpp::Rcout << "geometry_coordinates.length() " << geometry_coordinates.length() << std::endl;
  // Rcpp::Rcout << "start_indices.length() " << start_indices.length() << std::endl;
